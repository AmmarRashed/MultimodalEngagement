...
class EmoNet(nn.Module):
    def __init__(self, num_modules=2, n_expression=8, n_reg=2, n_blocks=4, attention=True, temporal_smoothing=False,
        ...
        self.truncate_output_layers = truncate_output_layers
        ...
        # self.out = nn.Sequential(nn.BatchNorm1d(128), nn.ReLU(inplace=True), nn.Linear(128, self.n_expression + n_reg))
        # last layer (emo_fc_2) was a sequential of 4 layers (linear - BatchNorm - relu - linear)
        # we keep the first linear as emo_fc_2, and the remaining 3 are used for a new layer called 'out'
        # we need to update this in the state_dict
        # def dict_updater(d, key_pairs):
        #     d = d.copy()
        ...
        #     if key.startswith("emo_fc_2"):
        #         layer_index = int(key.split(".")[1])
        #         if layer_index > 0:  # other than the first layer in emo_fc_2(linear)
        #             kp = (key, key.replace('emo_fc_2', 'out').replace(str(layer_index), str(layer_index - 1)))
        #             key_pairs.append(kp)
        #             print(f"{kp[0]}\t-->  {kp[1]}".expandtabs(32))
        # new_state_dict = dict_updater(state_dict, key_pairs)
    ...
    def forward(self, x, reset_smoothing=False):
        ...
        embedding = final_features.detach().clone().view(batch_size, -1)

        final_features = final_features.view(batch_size, final_features.shape[1])
        if self.truncate_output_layers:
